...
/*   Updated: 2023/11/22 13:15:04 by dhadding         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int in_here(t_cmd *cmd, int i)
{
	...
	cmd->fd_here = open(".temporary_heredoc", O_CREAT | O_RDWR | O_TRUNC, 0666);
	if (cmd->fd_here == -1)
		perror("Error: INFILE OPEN");
	write_2_fd_here(cmd);
...
int	out(t_cmd *cmd, int i)
{
	...
	cmd->fd_outfile = open(cmd->tokens[i + 1], O_CREAT | O_WRONLY | O_TRUNC, 0644);
	if (cmd->fd_outfile == -1)
...
{
	cmd->fd_stdout = dup(STDOUT_FILENO);
	cmd->fd_outfile = open(cmd->tokens[i + 1], O_CREAT | O_WRONLY | O_APPEND, 0644);
	if (cmd->fd_stdout == -1)
		perror("Error: OUTFILE OPEN");
	dup2(cmd->fd_outfile, STDOUT_FILENO);
	return (1);
